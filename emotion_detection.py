...
from geco import EmotionNet
import torch.nn.functional as nnf
from geco import utils
import numpy as np
import argparse
import torch
import cv2
...
parser.add_argument("-conf", "--confidence", type=int, default=0.6,
                    help="the minimum probability to filter out weak detection")
args = vars(parser.parse_args())

prototxt = os.path.join(os.getcwd(), 'model', 'deploy.prototxt.txt')
caffemodel = os.path.join(os.getcwd(), 'model', 'res10_300x300_ssd_iter_140000.caffemodel')
emotionModel = 'output/model.pth'
...
emotion_dict = {0: "Angry", 1: "Disgust", 2: "Fearful", 3: "Happy", 4: "Neutral",
                5: "Sad", 6: "Surprised", }

# load the emotionNet weights
model = EmotionNet(num_of_channels=1, num_of_classes=len(emotion_dict))
model.load_state_dict(torch.load(emotionModel))
model.to(device)
model.eval()
...
if args['video'] is not None:
    # If a video file path is provided, use it
    vs = cv2.VideoCapture(args['video'])
else:
    # If no video file path is provided, use the webcam (default is 0)
